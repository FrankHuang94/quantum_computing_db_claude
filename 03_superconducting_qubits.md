...
## Appendix B — Worked Comparison: Why Coherence-Time Records Don't Equal Algorithm Performance

A recurring confusion is conflating a *record T₁* with *device usefulness*. A fluxonium reporting T₁ > 1 ms is genuinely impressive, but algorithm performance depends on the full chain: two-qubit gate fidelity, connectivity, readout fidelity, crosstalk, and calibration stability across *all* qubits used, not the best single qubit's coherence. A device with 1 ms T₁ on one qubit but 99.0% two-qubit fidelity and heavy SWAP overhead from sparse connectivity may run a given algorithm *worse* than a device with 100 μs T₁ but 99.9% gates and better connectivity.

The right composite figures of merit (File 22) integrate these: quantum volume (rewards count × connectivity × fidelity jointly), application-specific benchmarks (run the actual target circuit), and — for the fault-tolerant horizon — the *logical* error rate after error correction, which depends most sensitively on the *worst* relevant two-qubit gate fidelity across the code patch, not the best coherence. This is why vendors increasingly report below-threshold logical-error scaling (Google Willow) rather than single-qubit coherence records: the former predicts fault-tolerant viability, the latter does not.

A concrete illustration: suppose device A has uniform 99.9% two-qubit fidelity across a square lattice, and device B has one spectacular 99.99% pair but a distribution with a long tail down to 99.0% and a sparse lattice. For a surface-code logical qubit (File 9), the logical error rate is set by the *typical-to-worst* physical error within the patch, so device A — uniform and well-connected — yields a far better logical qubit despite no record-setting individual number. **Uniformity and connectivity, not peak coherence, gate fault-tolerant value.** This principle, applied across modalities, is the analytical core of the cross-modality comparison in File 7 and the benchmarking critique in File 22, and it is the reason an engineer evaluating a superconducting (or any) processor should always ask for the *distribution* of two-qubit fidelities and the connectivity graph, never just the headline coherence or qubit count.

This closes the superconducting chapter. The physics (Josephson nonlinearity → transmon), the readout (dispersive circuit QED with quantum-limited amplification), the gates (tunable-coupler CZ/iSWAP and cross-resonance), the fabrication (lithographic films limited by TLS oxide loss), the named systems (IBM, Google, Rigetti, and the cat-qubit players), and the worked design calculations together equip the reader to evaluate any superconducting claim and to situate the modality within the broader hardware landscape developed in Files 4–7 and the infrastructure of Files 11 and 23.

> **Reader's takeaway for File 3.** When you encounter a superconducting-qubit claim — in a paper, a press release, or a vendor benchmark — decompose it along the axes this file established: the qubit design (transmon vs. fluxonium vs. cat) and its coherence-limiting mechanism (TLS, flux noise, quasiparticles); the gate scheme (CR vs. tunable-coupler CZ vs. parametric) and its fidelity *distribution*; the connectivity graph and its compilation cost; the readout chain and its single-shot fidelity; and the fabrication/materials lineage that sets the coherence ceiling. Then ask the File 22 question — does the reported metric predict performance on *your* target workload, or is it a random-circuit or single-qubit figure that flatters the architecture? That disciplined decomposition, applied consistently, is what separates engineering assessment from marketing absorption, and it is the habit this entire database aims to instill.
