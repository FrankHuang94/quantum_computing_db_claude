...
**(v) Operations-per-coherence, revisited.** Combining the numbers above: a 50 μs MS gate against a 1 s hyperfine T₂ gives T₂/t_gate ≈ 2×10⁴ two-qubit operations before decoherence — and against a demonstrated multi-minute T₂ (with clock transitions and dynamical decoupling), the ratio climbs above 10⁶. This is the single most important figure of merit (File 1, File 2 Section 25): it says a trapped-ion register can, in principle, execute tens of thousands to millions of high-fidelity two-qubit operations within its coherence envelope — a circuit depth vastly exceeding anything a NISQ superconducting device can sustain, and precisely the regime where deep, structured algorithms (Shor, quantum chemistry) and repeated error-correction cycles (File 9) become feasible. It is the quantitative heart of the trapped-ion value proposition, and the reason that, per-qubit, trapped ions remain the platform closest to the fault-tolerant regime even as their raw qubit counts trail neutral atoms (File 5) and superconducting (File 3) by one to two orders of magnitude.
