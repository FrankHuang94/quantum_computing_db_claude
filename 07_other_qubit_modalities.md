...
### 23. Reader's takeaway for File 7

These three modalities extend the strategic map beyond the leading platforms and sharpen the field's central insight: the choice of qubit is inseparable from the choice of error-correction strategy. Silicon spin qubits bet that CMOS manufacturing leverage (density, yield, cryo-CMOS co-integration) eventually outpaces per-qubit physics progress — a bet now backed by above-threshold fidelity and ms-scale ²⁸Si coherence but not yet by scale. Topological qubits bet that Majorana non-locality delivers hardware-level protection that slashes error-correction overhead — theoretically compelling but experimentally unresolved, and burdened by the 2018 retraction's cautionary lesson about how easily non-topological effects mimic the sought-after signatures. Bosonic/cat qubits bet that engineered biased noise reduces the outer code from a 2D surface code to a 1D repetition code — a mature enough idea to have reached break-even QEC, and pursued as a company-defining strategy by Alice & Bob and as an architecture by AWS. When you encounter any of these in a paper or roadmap, ask: what error structure does this encoding produce, what code is matched to it, and what does that imply for the physical-qubit overhead (File 18)? That question — error structure → matched code → overhead → timeline — is the thread connecting qubit choice to the path to utility, and it runs through every modality in Files 3–7 into the error-correction and resource-estimation analyses of Files 9 and 18.
