...
### A closing worked example: from notebook to hardware and back

To make the whole stack concrete in one arc: an engineer opens a Python notebook, imports Qiskit, and writes a five-line parameterized circuit for a small VQE problem (application → circuit layers). They call `transpile()` with optimization level 3 against a target IBM backend, and the transpiler maps their abstract qubits onto the device's heavy-hex lattice, inserts SWAPs for the non-adjacent interactions, decomposes to the native gate set, and schedules with dynamical decoupling on idle qubits (mapping → native-decomposition → pulse-adjacent layers, File 8). They wrap the run in a Qiskit Runtime session and call the Estimator primitive with ZNE mitigation enabled; the request travels to IBM's cloud, where it *queues*, then executes on the physical processor — control electronics (File 11) generating the microwave pulses, the qubits (File 3) evolving, the readout chain amplifying and digitizing — and the mitigation post-processing runs server-side (execution → control → hardware → mitigation layers, Files 10, 11). Expectation values return to the notebook, a SciPy SPSA optimizer updates the parameters, and the loop repeats until convergence (variational loop, Section 16). The engineer never touched a laser, a dilution refrigerator, a pulse, or a decoder — the software stack abstracted (leakily, but usably) the entire physical apparatus into a few notebook cells. That arc — notebook to cryogenic hardware and back, in a Python loop — is the software stack's achievement and its purpose: making the extraordinary physical machinery of Files 3–11 into something an engineer can actually program, and it is why the software stack is a co-equal pillar of the path to useful quantum computing.
