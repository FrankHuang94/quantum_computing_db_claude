...
## Appendix B — Worked Example: Comparing Two Advantage Claims

To exercise the assessment framework (Sections 34, 57), compare two hypothetical claims an engineer might encounter:

**Claim A: "Our quantum computer factored a 20-digit number, demonstrating progress toward breaking RSA."** Assessment: factoring a 20-digit (~64-bit) number is *classically trivial* (milliseconds on a laptop); the "progress toward RSA" framing is misleading because RSA-2048 is ~617 digits and requires *exponentially* more resources (millions of logical qubits, Section 4) — a 20-digit demonstration is ~9 orders of magnitude away in problem size and tells us almost nothing about cryptographically-relevant factoring. Moreover, many small "Shor" demonstrations used circuits with the answer compiled in (Section 35), not genuine period-finding. Verdict: technically real but *strategically meaningless* — a classic example of a demonstration whose significance is vastly overstated by the problem-size gap (Files 17, 22).

**Claim B: "Our variational quantum algorithm found lower-energy molecular configurations than classical DFT."** Assessment: apply the criteria (Section 34). Was the comparison against the *best* classical method (coupled-cluster, DMRG — not just DFT, which is known to struggle with strong correlation)? Section 57: is this a provable, conjectured, or heuristic claim (VQE advantage is heuristic, Section 17)? Was barren-plateau-free training achieved at this scale (Section 16)? What was the measurement/shot cost (Section 28)? Is the molecule small enough that classical methods handle it easily (Section 13)? Verdict: requires careful scrutiny — VQE beating *DFT* (a weak baseline for correlated systems) is unremarkable; VQE beating the *best* classical method (coupled-cluster/DMRG) at a scale those methods can't reach would be significant but is not what current NISQ VQE achieves (Section 17). The claim likely overstates advantage by choosing a weak classical baseline — the exact pattern the classical-comparison discipline (File 14) guards against.

These worked comparisons show the assessment framework in action: for *any* quantum-advantage claim, ask about the problem size relative to classical tractability, the strength of the classical baseline actually run (not just cited), the provable/conjectured/heuristic status, and the honest accounting of overhead (T-count, measurement, state-prep). This disciplined interrogation — the algorithm-engineering counterpart to the hardware-assessment discipline of Files 3–7, the QEC-assessment discipline of File 9, and the benchmarking skepticism of File 22 — is the practical skill this file aims to instill, and it is what separates rigorous evaluation of quantum-algorithmic progress from absorption of hype. An engineer who can perform this interrogation for any claim they encounter — distinguishing the genuine (exponential, structured, benign-I/O, awaiting fault tolerance) from the overstated (small-problem demonstrations, weak-baseline comparisons, heuristic NISQ claims, dequantizable data-driven speedups) — has internalized the core lesson of the quantum-algorithms landscape and is equipped to navigate the field's promise and hype with the honesty the whole database aims to model.

> **Final reader's takeaway for File 13.** Quantum algorithms are interference engines for structured problems, not general-purpose accelerators. The genuine, transformative, *proven* advantages — Shor's factoring and quantum simulation — are exponential, structured, and awaiting fault-tolerant hardware; Grover's search is a real but overhead-limited quadratic speedup; and the NISQ-era variational and QML algorithms that run today have unproven, contested advantage. Carry the five criteria (large speedup, exploitable structure, benign I/O, sufficient interference, manageable T-count) and the provable/conjectured/heuristic taxonomy as your assessment tools, demand the best classical baseline actually run, and ground every claim in the resource estimation of File 18 — and you will read the quantum-algorithms landscape, and its pervasive hype, with the honesty this database models throughout.
