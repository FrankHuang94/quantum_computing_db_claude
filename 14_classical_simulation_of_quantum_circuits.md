...
Classical simulation — state-vector (exact, ~40-qubit ceiling), tensor-network (limited-entanglement), stabilizer (Clifford, T-count governs hardness), and hybrid/fidelity-matched methods — is quantum computing's moving-target benchmark (repeatedly catching up to supremacy and utility claims) and essential development infrastructure, governed by entanglement/T-count/depth (not qubit count), teaching that advantage claims are provisional and durable advantage likely requires fault tolerance to run the high-complexity circuits classical methods cannot cheaply follow.
