...
Quantum networking distributes entanglement between separated nodes for QKD (BB84/E91, deployable now over fiber and satellite but a niche complement to mainstream PQC, File 21), distributed/modular computing (Files 4, 6, 11), and networked sensing (File 16) — gated by the unsolved quantum-memory and high-rate-interconnect challenges that make a full repeater-based quantum internet a multi-decade effort, with deep, strategically-important synergy with photonic computing (File 6).

### Extended note: the memory bottleneck as the pacing item

Among quantum networking's challenges, the *quantum memory* (Section 6) is the pacing item for the quantum internet. Repeaters need memories that hold a qubit coherently through many entanglement-generation attempts across an adjacent link — requiring high storage fidelity, long coherence, efficient photon interfaces, and fast retrieval, simultaneously. Candidate memories (NV centers, rare-earth-doped crystals, atomic ensembles, trapped ions/atoms, File 15) each optimize some of these at the expense of others, and no memory yet meets all requirements at the performance and scale a practical repeater chain demands. This is why entanglement-distribution *demonstrations* (Micius, metro testbeds, Section 7) are proof-of-principle rather than scalable infrastructure, and why the full quantum internet (Section 8) is a multi-decade effort. Tracking quantum-memory progress (coherence, fidelity, efficiency) is thus the key indicator for the networking timeline — the memory bottleneck gates the repeater, which gates the quantum internet, which gates the long-horizon networking applications (blind computing, distributed computing, networked sensing, Sections 18–19). Near-term, the practical networking reality remains point-to-point QKD (fiber and satellite) and small metro testbeds, with the transformative repeater-based internet awaiting the memory breakthrough.
