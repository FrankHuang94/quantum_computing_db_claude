...
Quantum sensing (atomic clocks, NV/SQUID/atomic magnetometers, cold-atom gravimeters) is the mature, deployed, revenue-generating face of quantum technology — exploiting the *easy* part of quantum control (coherent single-/few-system precision) that computing shares but goes beyond — providing the calibration that realized quantum advantage exists (in sensing), the shared control technology is validated, and computing's remaining challenge is specifically scalable error correction (Files 9, 18), not quantum control itself.

### Extended note: the sensing-computing maturity gap explained

The fundamental reason quantum *sensing* is commercially mature while quantum *computing* is not (Section 5) is the difference in requirements. Sensing exploits *coherent phase accumulation* in a *single or few* quantum systems to measure a physical quantity with precision — a task that needs excellent *coherence and control* but *not* many-qubit entanglement, error correction, or scalability. A single NV center, one clock ion, or a small atom ensemble suffices, and the same Ramsey-interferometry and coherent-control techniques (Section 7) that computing uses for *gates* here serve *measurement* — with the crucial difference that measurement needs no scalable error-corrected many-qubit processor. Computing, by contrast, needs *scalable, error-corrected, many-qubit control* (Files 9, 11, 18) — vastly harder. So sensing succeeds with the *easy* part of quantum control (coherent precision) while computing needs the *hard* part (scalable error correction) — which is why sensing delivers realized commercial advantage today while computing remains a longer-term prospect. This maturity gap is fundamental, not incidental, and it precisely diagnoses computing's remaining challenge as *scaling and error correction* (Files 9, 18), not quantum control per se — a diagnosis the sensing success validates by proving the underlying control technology (lasers, trapping, readout) works and is deployed.
