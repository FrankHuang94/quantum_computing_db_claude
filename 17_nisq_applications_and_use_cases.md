...
## Appendix — The NISQ-Assessment Checklist (Reference Card)

For *any* near-term quantum-advantage claim, ask:

1. **Best classical baseline:** what is the *best current* classical algorithm for this *exact* instance (tensor networks, specialized solvers, GPU methods, File 14) — not a naive or the-company's-old baseline?
2. **Actually run vs. cited:** was that classical method *run* for comparison, or only *cited* as an asymptotic-complexity argument (which is frequently overturned, Files 10, 14)?
3. **Problem size vs. crossover:** how does the demonstrated size compare to the classical-quantum crossover — is it a size classical methods handle easily?
4. **Useful vs. contrived:** is the task genuinely useful, or a contrived sampling problem (Files 6, 14) whose "advantage" is a physics demonstration, not a practical achievement?
5. **Reproduced:** has the result been independently replicated/verified (File 22)?
6. **Full-stack accounting:** does the claim account for error-mitigation overhead (File 10), state preparation, readout, and compilation cost (File 12) — or just the "quantum" part?
7. **Baseline strength:** is "advantage" measured against the *best* classical method or a *weak* one (the most common overclaim, e.g., beating DFT rather than coupled-cluster for a correlated system)?

Applying this checklist separates the rare genuine benchmarked advantage from the theoretically-motivated (the honest majority) and the overhyped (unfortunately common). The honest bottom line across all NISQ sectors — chemistry, optimization, ML, finance, pharma — is that **no broadly-accepted, reproducible, practically-significant advantage over the best classical methods has been demonstrated on a useful problem**, and near-term value lies in capability-building and scientific exploration (analog simulation being the strongest case), not demonstrated advantage. The transformative, proven advantages (Shor, useful simulation, File 13) await fault tolerance (File 18). This skeptical, evidence-based, classically-benchmarked discipline — the antidote to the hype cycle (File 24) — is the honest calibration this file contributes to the database's assessment of what near-term quantum computing can and cannot do.

### Extended note: why the low-complexity vise binds

The deepest reason near-term advantage is elusive (Files 10, 14) is the *low-complexity vise*: error mitigation (File 10) and NISQ hardware handle circuits of *limited* depth, entanglement, and non-Cliffordness — but those are *precisely* the circuits that advanced classical methods (tensor networks, sparse Pauli dynamics, File 14) also handle. To escape the vise, one needs high-entanglement, high-T-count, deep circuits that classical methods *cannot* follow — but those circuits are *beyond* noisy NISQ capability (they accumulate too much error) and beyond mitigation's exponential-sampling reach (File 10). So genuine, durable advantage on a *useful* problem requires *fault tolerance* (File 9) to run the high-complexity circuits that both defeat classical simulation and exceed NISQ error budgets. This vise (Files 10, 14) is why the honest near-term conclusion is capability-building and exploration, not demonstrated advantage — and why the transformative, proven advantages (Shor, useful simulation, File 13) are fault-tolerant-era prospects (File 18). Recognizing the vise — that the circuits NISQ can run are the circuits classical methods can simulate — is the single most clarifying insight for calibrating near-term expectations, and it underlies the skeptical assessment this file models throughout.
