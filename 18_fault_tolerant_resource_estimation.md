...
### 56. A final worked comparison: three timelines for RSA

To crystallize the assumption-dependence and the timeline uncertainty, consider three RSA-2048 scenarios that resource estimation produces from different assumptions:

- **Pessimistic:** surface code, p=10⁻³, no overhead-reduction levers, honest infrastructure accounting → ~20M+ physical qubits plus the full cryo-CMOS/decoder/interconnect infrastructure (File 11) → feasible only in the *late 2030s or beyond*, if the infrastructure scaling is solved.
- **Central:** modest fidelity improvement (p~5×10⁻⁴), partial qLDPC adoption, improved distillation → ~a few million physical qubits → plausibly *2035-ish* if hardware and codes progress on trend.
- **Optimistic:** low error rate (p~10⁻⁴), full qLDPC, biased-noise or erasure hardware, magic-state cultivation → ~1M physical qubits → conceivably *early 2030s* if all the levers deliver.

The ~20× spread in qubit count and the ~decade spread in timeline across these scenarios is *not* a failure of resource estimation — it is resource estimation *correctly* reporting that the answer depends on which overhead-reduction levers deliver (Section 12). The scenarios also show what to *track*: the physical error rate, qLDPC adoption, biased-noise/erasure maturity, distillation improvements, and infrastructure scaling — each a milestone that, as it delivers or fails, moves the estimate toward the optimistic or pessimistic end. This is the mature way to hold the "when will RSA fall?" question (File 21): not a single date, but a scenario range with identified levers, updated as the levers resolve. It exemplifies resource estimation's core contribution — replacing a single speculative number with a rigorous, assumption-transparent, lever-identified range that supports sound decision-making about the cryptographic threat (File 21), investment (File 24), and the path to utility (File 19). The honest bottom line: a cryptographically-relevant quantum computer is a *real but not imminent* prospect, its timeline a shrinking range gated by specific, trackable engineering levers — precisely the grounded, quantitative, honestly-uncertain assessment that resource estimation exists to provide.

> **One-sentence capstone.** Resource estimation is the discipline that composes an algorithm's logical qubits and T-count (File 13), the fault-tolerant code overhead and dominant magic-state-factory cost (File 9), the physical error rate and clock speed of the hardware (Files 3–7), and the classical control/decoding/cryogenic infrastructure that must scale alongside (File 11) into a concrete, assumption-transparent, shrinking-over-time range for the physical-qubit count, runtime, and application-feasibility crossover — quantifying the large-but-closing gap from today's handful of logical qubits to the thousands that useful applications require, disqualifying overhead-swamped quadratic speedups while confirming exponential ones, and grounding the honest, lever-identified, 2030s-and-beyond timeline that is the disciplined middle between quantum-computing hype and dismissal.

Every subsequent strategic file in this database — the roadmap-credibility methodology (File 19), the geopolitics of the cryptographic transition (File 21), the market and investment reality (File 24), and the frontier overhead-reduction research (File 25) — draws on the resource-estimation framework developed here, using its numbers, its levers, and its honest-uncertainty framing to assess quantum computing's genuine, quantifiable, and hard-but-not-impossible path to utility.
