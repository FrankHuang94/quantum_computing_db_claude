...
Assess quantum-hardware roadmaps by track record on past milestones, peer-reviewed vs. press-release claims, independent benchmarking, metric-shifting awareness, and demonstrated-vs-projected separation — crediting the strongest *demonstrated* results (Google's below-threshold error correction, Quantinuum's record quantum volume and logical qubits, QuEra's 48 logical qubits, File 9) while discounting aggressive projected timelines (industry slippage is the norm) and reading logical-qubit targets and resource-estimated application crossovers (File 18), not raw physical qubit counts.

### Extended note: why demonstrated beats projected

The core roadmap-assessment principle (Part I) — credit *demonstrated* results, discount *projected* milestones — rests on the field's history of timeline slippage (Section 15). Aggressive "useful quantum computing in N years" predictions have repeatedly been revised outward, not because the technology failed but because the engineering (Files 9, 11, 18) proved harder than optimistic projections assumed. Meanwhile, the *demonstrated* results — Google's below-threshold error correction, Quantinuum's record quantum volume, QuEra's 48 logical qubits (File 9) — are *facts* that advanced the field regardless of any timeline. So an analyst assessing a roadmap should treat the demonstrated results as the reliable signal of capability, and the projected milestones as claims to be weighed against the company's track record and the resource-estimation-grounded feasibility (File 18) — defaulting to skepticism on aggressive near-term timelines while crediting the real, peer-reviewed progress. This demonstrated-over-projected discipline, combined with awareness of metric-shifting (favorable composite metrics, File 22) and the reframing toward logical-qubit and application-crossover targets (File 18), is the durable framework for reading any roadmap, current or future — the skeptical, evidence-based assessment that separates ambition (roadmaps) from capability (demonstrated results).
