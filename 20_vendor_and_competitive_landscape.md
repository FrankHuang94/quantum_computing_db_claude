...
> **One-paragraph summary of File 20.** The quantum-computing vendor landscape comprises vertically-integrated giants (IBM's Qiskit-ecosystem enterprise leadership; Google's below-threshold-error-correction science leadership), platform/marketplace giants hedged across modalities (Microsoft Azure Quantum + topological moonshot; AWS Braket + cat qubits), pure-play hardware companies (Quantinuum's strong trapped-ion + software + Honeywell-backed position; IonQ's visibility with metric caveats; D-Wave's contested-advantage annealing deployment; Rigetti's smaller superconducting play), specialized single-architecture bets (PsiQuantum's high-variance government-backed photonic moonshot; the fast-rising neutral-atom cohort of QuEra/Pasqal/Atom Computing; Xanadu's photonic + PennyLane; Alice & Bob's cat qubits), and resilient modality-agnostic picks-and-shovels software firms (Q-CTRL, Riverlane, Classiq) — all competing for *position* ahead of a *future* fault-tolerant market (File 18) in a pre-revenue-at-scale, capital-intensive, talent-concentrated, consolidating, and geopolitically-entangled sector where the winner depends on the undecided cross-modality race, demonstrated results beat roadmaps, the most durable moats are ecosystem/software and capital, and the technology's real progress and the business's long uncertain commercial timeline must be held in view together — the honest, evidence-based competitive assessment the database models throughout.
